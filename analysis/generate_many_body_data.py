...
from utils import load_dataframe, load_model
...
parser = argparse.ArgumentParser(prog="Generate Many-Body Data",
                                 usage='%(prog)s [options]',
                                 description="Computes many-body observables via MCMC sampling",
                                 epilog="and fin")
...
parser.add_argument("--optim_name",         type=str,   default='Adam', help='Name of numerical optimiser used.')
...
optim_name = args.optim_name #optim.__class__.__name__
...
optim_name = 'QNKFAC-VMC-VMC-MR-50' #torch.optim.Adam(params=net.parameters(), lr=1e-4) #new optimizer
...
#output_dict = load_model(model_path=model_path, device=device, net=net, optim=optim_name, sampler=sampler)
...
with torch.no_grad():
    for batch in tqdm(range(nbatches), desc="Generating configurations"):
        #Ghost-particle method for computing one-body density matrix [DOI: 10.1103/PhysRev.138.A442]
        x, _ = sampler(n_sweeps=n_sweeps)

        s = torch.rand_like(x)*(xmax-xmin) + xmin #uniform rand in range [xmin, xmax)

        configurations[batch, :, :] = x.detach().clone()
...
analysis_datapath = DIR+"analysis/PHYS_A%02i_H%03i_L%02i_D%02i_%s_W%04i_P%06i_V%4.2e_S%4.2e_%s_PT_%s_device_%s_dtype_%s.npz" % \
                (nfermions, num_hidden, num_layers, num_dets, func.__class__.__name__, nwalkers, preepochs, V0, sigma0, \
                 optim_name, False, device, dtype)
