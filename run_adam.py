...
parser = argparse.ArgumentParser(description='SpinlessFermions')
...
gs_CI = get_groundstate(A=nfermions, V0=V0, datapath=DIR+"groundstate/")
...
for epoch in range(start, epochs+1):
    ...
    sys.stdout.write("Epoch: %6i | Energy: %6.4f +/- %6.4f | CI: %6.4f | HF: %6.4f | Walltime: %4.2e (s)        \r" % (epoch, energy_mean, (energy_var / nwalkers).sqrt(), gs_CI, enerhf, t1-t0))
    sys.stdout.flush()
...
nbatches = 10_000
energy_mean, error_of_mean = generate_final_energy(calc_elocal=calc_elocal,
                                                   sampler=sampler,
                                                   nbatches=nbatches,
