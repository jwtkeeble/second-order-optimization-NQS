...
parser = argparse.ArgumentParser(description='QuasiNewton-KFAC')
...
# We have 2 copies of the energy estimator (as one cannot compute derivatives for an nn.Module object that has backward hooks)
...
# From optimizer_qnkfac import QNKFAC
from qnkfac_optimiser import QNKFAC

optim = QNKFAC(model=net,
...
for epoch in range(start, epochs+1):
    ...
    stats['max_update'] = max([torch.max(p.grad.abs()).item() for p in net.parameters()])  # TODO rename with l_infinity norm or sth
...
from utils import generate_final_energy, round_to_err, str_with_err
nbatches = 10_000
energy_mean, error_of_mean = generate_final_energy(calc_elocal=calc_local_no_hooks,
                                                   sampler=sampler,
                                                   nbatches=nbatches,
                                                   chunk_size=None, #full-batch vectorization
                                                   n_sweeps=10,     #10 is fine, 400 is too much 
                                                   storage_device=torch.device('cpu')) #store on cpu to save memory for GPU
x, dx = round_to_err(energy_mean.item(), error_of_mean.item())
energy_str = str_with_err(x, dx)
print(f"Energy: {energy_str}")

final_energy_str = DIR+"results/final/FINAL_A%02i_H%03i_L%02i_D%02i_%s_W%04i_P%06i_V%4.2e_S%4.2e_%s_PT_%s_device_%s_dtype_%s.csv" % \
                (nfermions, num_hidden, num_layers, num_dets, func.__class__.__name__, nwalkers, preepochs, V0, sigma0, optim_name, False, device, dtype)
print(f"Saving to {final_energy_str}")
data = {'energy_mean': x, 'error_of_mean': dx, 'energy_str':energy_str}
import pandas as pd
df = pd.DataFrame(data=data, index=[0])
df.to_csv(final_energy_str, sep='\t', encoding='utf-8')
"""